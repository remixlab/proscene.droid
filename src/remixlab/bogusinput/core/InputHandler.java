...
/**
 * Third-parties (like the <b>remixlab.dandelion</b> package) using the <b>remixlab.bogusinput</b> package should
 * instantiate a single InputHandler object which is the high level package handler. The handler holds a collection of
 * {@link #agents()}, and an event dispatcher queue of {@link remixlab.bogusinput.core.EventGrabberTuple}s (
 * {@link #eventTupleQueue()}). Such tuple represents a message passing to application objects, allowing an object to be
 * instructed to perform a particular user-defined {@link remixlab.bogusinput.core.Action} from a given
 * {@link remixlab.bogusinput.event.BogusEvent}.
 * <p>
 * At runtime, the input handler should continuously runs the two loops defined in {@link #handle()}. Therefore, simply
...
public class InputHandler {
	 ...
	 * <p>
	 * 1. {@link remixlab.bogusinput.core.EventGrabberTuple} producer loop which for each registered agent calls:
	 * {@link remixlab.bogusinput.core.Agent#handle(BogusEvent)}. Note that the bogus event is obtained from the agents
	 * callback {@link remixlab.bogusinput.core.Agent#feed()} method <br>
	 * 2. User-defined action consumer loop: which for each {@link remixlab.bogusinput.core.EventGrabberTuple} calls
	 * {@link remixlab.bogusinput.core.EventGrabberTuple#perform()}.<br>
	 */
	public void handle() {
		// 1. Agents
	...
	/**
	 * Adds {@code grabber} to the {@code agent} {@link remixlab.bogusinput.core.Agent#pool()}.
	 */
	...
	/**
	 * Removes {@code grabber} from the {@code agent} {@link remixlab.bogusinput.core.Agent#pool()}.
	 */
	...
	/**
	 * Clears the {@code agent} {@link remixlab.bogusinput.core.Agent#pool()}.
	 */
	...
	/**
	 * Clears all registered agent's {@link remixlab.bogusinput.core.Agent#pool()}.
	 */
