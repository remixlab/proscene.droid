...
/**
 * Grabbables are means to attach a set of user-defined {@link remixlab.bias.core.Action} groups to application
 * objects. Grabbers are attached to {@link remixlab.bias.core.Agent}s through their API, and may be attached to
 ...
 * which defines the rules to set the application object as the agents {@link remixlab.bias.core.Agent#grabber()};
 * and, {@link #performInteraction(BogusEvent)}, which defines how the application object should behave according to a
 * given bogus event, which may be parameterized to hold a user-defined action.
 */
